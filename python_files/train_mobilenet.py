...
def train_mobilenet(num_neurons, batch_size, num_epochs, scaling, save_dir, y_train, train_dataloader, pretrained, learning_rate, train_proportion):
    mobile_net_model = mobile_net(pretrained, num_neurons)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    mobile_net_model.to(device)

    optimizer = optim.Adam(mobile_net_model.parameters(), lr=learning_rate)
    ...
    for epoch in range(num_epochs):
        ...
        save_training_states(save_dir, mobile_net_model, optimizer, scheduler, epoch, loss.item(), train_proportion, num_neurons, scaling, sequence_length=None, hidden_size=None)
